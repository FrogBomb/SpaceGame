...
class tileSprite(animatedSprite):
    def __init__(self, images, position, tileType = 0, isAnimated = False, fps=10):
        pygame.sprite.Sprite.__init__(self)
        ...
        self.image = images[self._curRow][isAnimated]
        self.update(0)
    def update(self, t, tileType = None, toggleAnimation = False):
        if toggleAnimation:
            self._isAnimated = not self._isAnimated
        if tileType != None:
            self._curRow = tileType
        if self._isAnimated:
            if t - self._last_update > self._delay:
                self._frame += 1
                self._last_update = t
